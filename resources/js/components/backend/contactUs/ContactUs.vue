              ...
              <div class="col-12">
                ...
                <div class="card datatable-background">
                  <div class="card-header">
                    <h3 class="card-title text-light font-weight-bold">Total Contact-Us </h3>

                    <button type="button" class="btn btn-secondary float-right" data-toggle="modal" @click="addContactUs">Add-New <i class="nav-icon fas fa-plus ml-2"></i></button>
                  </div>
                ...
                <div class="modal-dialog modal-lg">
                  <div class="modal-content bg-primary">
                    <div class="modal-header modal-modify">
                      ...
                      <!-- form start -->
                    <form @submit.prevent="editMode ? update() : createContactUs()" @keydown="form.onKeydown($event)">
                      <div class="modal-body">
                        ...
                        <div class="form-group mb-1">
                            <label>Touch Description</label>
                            <textarea v-model="form.touch_description" type="text" name="touch_description"
                                class="form-control" :class="{ 'is-invalid': form.errors.has('touch_description') }" placeholder="Description"></textarea>
                            <has-error :form="form" field="touch_description"></has-error>
                        </div>
                        <div class="form-group mb-1">
              ...
              <div class="card-body"> 
                  <table class="table table-secondary table-bordered table-striped text-center">
                    <thead>
                      ...
                      <tr v-for="(result, index) in contactUs" :key="result.id">
                        <td>{{ result.id }}</td>
                        <td>{{ result.title }}</td>
                        <td>{{ result.description }}</td>
                        <td>
                            <img :src="'/uploads/contactUs/'+result.image" alt="No image" style="height: 60px; width: 60px;border-radius:50%;">
                        </td>
                        <td style="width:15%;">
...
<script>
    ...
    export default {
        ...
        methods: {
            getData() {
                axios.get('/api/contactUs?page='+this.pagination.current_page)
                .then(response => {
            ...
            searchData(){
              axios.get('/api/search/contactUs/'+this.queryField+'/'+this.query+'?page='+this.pagination.current_page)
                .then(response => {
                    this.contactUs=response.data.data;
                    this.pagination=response.data.meta;
                })
            ...
            addContactUs() {
                $('#addItem').modal('show');
                $('#addModalLabel').text('Add Contact-Us..');
            ...
            createContactUs() {
              this.form.busy = true;
              this.form.post('/api/contactUs')

              .then(response => {
                    this.getData();
                    if(this.form.successful){
                        $('#addItem').modal('hide');
                        toastr.success('Successfully added contactUs.', 'Success')
                    }
                })
            ...
            edit(result) {
                ...
                $('#addItem').modal('show');
                $('#addModalLabel').text('Update Contact-Us..');
                $('#buttonText').text('Update-Data');
            },

            update(){
                this.form.busy = true;
                this.form.patch('/api/contactUs/'+this.form.id)
                .then(response => {
                    this.getData()
                    if(this.form.successful){
                    $('#addItem').modal('hide');
                        toastr.success('ContactUs Updated Successfully.', 'Success')
                    }
                })
                .catch(e => {
                    toastr.error('Something is missing...!', 'Prepare to die!')
            ...
            active(id) {
                axios.get('/api/contactUs/active/'+id)

                .then(response => {
                this.getData();
                    toastr.success('Successfully contactUs activated.', 'Success')
                })
            ...
            unactive(id) {
                axios.get('/api/contactUs/unactive/'+id)

                .then(response => {
                    this.getData();
                    toastr.success('Successfully contactUs un-activated.', 'Success')
                })
            ...
            destroy(result) {
                ...
                $('#deleteModalLabel').text('Delete ContactUs Info..');
            },

            destoryInfo() {
                this.form.delete('/api/contactUs/'+this.form.id)
                .then(response => {
                this.getData();
                $('#deleteModal').modal('hide');
                if(this.form.successful){
                        toastr.success('Successfully contactUs deleted.', 'Success')
                    }
                })
                .catch(e => {
                    toastr.error('Something is wrong...!', 'Prepare to die!')
