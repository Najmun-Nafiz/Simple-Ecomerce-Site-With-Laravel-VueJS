              ...
              <div class="col-12">
                ...
                <div class="card datatable-background">
                  <div class="card-header">
                    <h3 class="card-title text-light font-weight-bold">Total Sellers </h3>

                    <button type="button" class="btn btn-secondary float-right" data-toggle="modal" @click="addSeller">Add-New <i class="nav-icon fas fa-plus ml-2"></i></button>
                  </div>
                ...
                <div class="modal-dialog modal-lg">
                  <div class="modal-content bg-primary">
                    <div class="modal-header modal-modify">
                      ...
                      <!-- form start -->
                    <form @submit.prevent="editMode ? update() : createSeller()" @keydown="form.onKeydown($event)">
                      <div class="modal-body">
                        ...
                        <div class="form-group mb-1">
                            ...
                            <input v-model="form.profession" type="text" name="profession"
                                class="form-control" :class="{ 'is-invalid': form.errors.has('profession') }" placeholder="Profession">
              ...
              <div class="card-body"> 
                  <table class="table table-secondary table-bordered table-striped text-center">
                    <thead>
                      ...
                      <tr v-for="(result, index) in sellers" :key="result.id">
                        <td>{{ result.id }}</td>
                        <td>{{ result.title }}</td>
                        <td>{{ result.description }}</td>
                        <td>
                            <img :src="'/uploads/seller/'+result.image" alt="No image" style="height: 60px; width: 60px;border-radius:50%;">
                        </td>
                        <td style="width:15%;">
...
<script>
    ...
    export default {
        ...
        methods: {
            getData() {
                axios.get('/api/seller?page='+this.pagination.current_page)
                .then(response => {
            ...
            searchData(){
              axios.get('/api/search/seller/'+this.queryField+'/'+this.query+'?page='+this.pagination.current_page)
                .then(response => {
                    this.sellers=response.data.data;
                    this.pagination=response.data.meta;
                })
            ...
            createSeller() {
                this.form.busy = true;
                this.form.post('/api/seller')

                .then(response => {
                    this.getData();
                    if(this.form.successful){
                        $('#addItem').modal('hide');
                        toastr.success('Successfully added seller.', 'Success')
                    }
                })
            ...
            edit(result) {
                ...
                $('#addItem').modal('show');
                $('#addModalLabel').text('Update Seller..');
                $('#buttonText').text('Update-Data');
            },

            update(){
                this.form.busy = true;
                this.form.put('/api/seller/'+this.form.id)
                .then(response => {
                    this.getData()
                    if(this.form.successful){
                    $('#addItem').modal('hide');
            ...
            active(id) {
                axios.get('/api/seller/active/'+id)

                .then(response => {
                this.getData();
                    toastr.success('Successfully seller activated.', 'Success')
                })
            ...
            unactive(id) {
                axios.get('/api/seller/unactive/'+id)

                .then(response => {
                    this.getData();
                    toastr.success('Successfully seller un-activated.', 'Success')
                })
            ...
            destroy(result) {
                ...
                $('#deleteModalLabel').text('Delete Seller Info..');
            },

            destoryInfo() {
                this.form.delete('/api/seller/'+this.form.id)
                .then(response => {
                this.getData();
                $('#deleteModal').modal('hide');
                if(this.form.successful){
                        toastr.success('Successfully seller deleted.', 'Success')
                    }
                })
                .catch(e => {
                    toastr.error('Something is wrong...!', 'Prepare to die!')
