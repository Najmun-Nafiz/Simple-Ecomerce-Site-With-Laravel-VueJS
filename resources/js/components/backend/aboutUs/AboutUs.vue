              ...
              <div class="col-12">
                ...
                <div class="card datatable-background">
                  <div class="card-header">
                    <h3 class="card-title text-light font-weight-bold">Total About-Us </h3>

                    <button type="button" class="btn btn-secondary float-right" data-toggle="modal" @click="addAboutUs">Add-New <i class="nav-icon fas fa-plus ml-2"></i></button>
                  </div>
                ...
                <div class="modal-dialog modal-lg">
                  <div class="modal-content bg-primary">
                    <div class="modal-header modal-modify">
                      ...
                      <!-- form start -->
                    <form @submit.prevent="editMode ? update() : createAboutUs()" @keydown="form.onKeydown($event)">
                      <div class="modal-body">
                        ...
                        <div class="form-group mb-1">
                            <label>Slect Back Image</label>
                            <input type="file" class="form-control" id="inputImage" name="back_image" @change="onChanged" :class="{ 'is-invalid': form.errors.has('back_image') }" placeholder="Photo">
                            <has-error :form="form" field="back_image"></has-error>
                        </div>
                      </div>
              ...
              <div class="card-body"> 
                  <table class="table table-secondary table-bordered table-striped text-center">
                    <thead>
                      ...
                      <tr v-for="(result, index) in aboutUs" :key="result.id">
                        <td>{{ result.id }}</td>
                        <td>{{ result.title }}</td>
                        <td>{{ result.description }}</td>
                        <td>
                            <img :src="'/uploads/aboutUs/'+result.image" alt="No image" style="height: 60px; width: 60px;border-radius:50%;">
                        </td>
                        <td style="width:15%;">
...
<script>
    ...
    export default {
        ...
        methods: {
            getData() {
                axios.get('/api/aboutUs?page='+this.pagination.current_page)
                .then(response => {
            ...
            searchData(){
              axios.get('/api/search/aboutUs/'+this.queryField+'/'+this.query+'?page='+this.pagination.current_page)
                .then(response => {
                    this.aboutUs=response.data.data;
                    this.pagination=response.data.meta;
                })
            ...
            addAboutUs() {
                $('#addItem').modal('show');
                $('#addModalLabel').text('Add About-Us..');
            ...
            onChanged(e){
                let back_image = e.target.files[0];
                let reader = new FileReader();
                reader.readAsDataURL(back_image);
                reader.onload = (e) => {
                    this.form.back_image = e.target.result
                }
            },

            createAboutUs() {
              	this.form.busy = true;
              	this.form.post('/api/aboutUs')

              	.then(response => {
                    this.getData();
                    if(this.form.successful){
                        $('#addItem').modal('hide');
                        toastr.success('Successfully added aboutUs.', 'Success')
                    }
                })
            ...
            edit(result) {
                ...
                $('#addItem').modal('show');
                $('#addModalLabel').text('Update About-Us..');
                $('#buttonText').text('Update-Data');
            },

            update(){
                this.form.busy = true;
                this.form.put('/api/aboutUs/'+this.form.id)
                .then(response => {
                    this.getData()
                    if(this.form.successful){
                    $('#addItem').modal('hide');
                        toastr.success('AboutUs Updated Successfully.', 'Success')
                    }
                })
                .catch(e => {
                    toastr.error('Something is missing...!', 'Prepare to die!')
            ...
            active(id) {
                axios.get('/api/aboutUs/active/'+id)

                .then(response => {
                this.getData();
                    toastr.success('Successfully aboutUs activated.', 'Success')
                })
            ...
            unactive(id) {
                axios.get('/api/aboutUs/unactive/'+id)

                .then(response => {
                    this.getData();
                    toastr.success('Successfully aboutUs un-activated.', 'Success')
                })
            ...
            destroy(result) {
                ...
                $('#deleteModalLabel').text('Delete AboutUs Info..');
            },

            destoryInfo() {
                this.form.delete('/api/aboutUs/'+this.form.id)
                .then(response => {
                this.getData();
                $('#deleteModal').modal('hide');
                if(this.form.successful){
                        toastr.success('Successfully aboutUs deleted.', 'Success')
                    }
                })
                .catch(e => {
                    toastr.error('Something is wrong...!', 'Prepare to die!')
