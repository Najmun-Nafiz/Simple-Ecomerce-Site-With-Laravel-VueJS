	          ...
	          <div class="col-12">
	            ...
	            <div class="card datatable-background">
	              <div class="card-header">
	                <h3 class="card-title text-light font-weight-bold">Total Messages </h3>
	              </div>
	            ...
	            <div class="card-body"> 
	              <table class="table table-secondary table-bordered table-striped text-center">
	                <thead>
	                  ...
	                  <tr v-for="(result, index) in messages" :key="result.id">
	                    <td>{{ result.id }}</td>
...
<script>
    ...
    export default {
        ...
        methods: {
			...
			getData(){
				axios.get('/api/message?page='+this.pagination.current_page)
				.then(response => {
			...
			searchData(){
	              axios.get('/api/search/message/'+this.queryField+'/'+this.query+'?page='+this.pagination.current_page)
	        ...
	        active(id) {
	          axios.get('/api/message/active/'+id)

	          .then(response => {
	              this.getData();
	              toastr.success('Successfully message activated.', 'Success')
	            })
	        ...
	        unactive(id) {
	          axios.get('/api/message/unactive/'+id)

	          .then(response => {
	              this.getData();
	              toastr.success('Successfully message un-activated.', 'Success')
	            })
	        ...
	        destroy(result) {
				...
				$('#deleteModalLabel').text('Delete Message Info..');
	        },

	        destoryInfo() {
	          	this.form.delete('/api/message/'+this.form.id)
	          	.then(response => {
		            this.getData();
		            $('#deleteModal').modal('hide');
		            if(this.form.successful){
		              	toastr.success('Successfully message deleted.', 'Success')
		            }
	          })
	          .catch(e => {
	            	toastr.error('Something is wrong...!', 'Prepare to die!')
