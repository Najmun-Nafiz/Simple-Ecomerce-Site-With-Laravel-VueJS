	          ...
	          <div class="col-12">
	            ...
	            <div class="card datatable-background">
	              <div class="card-header">
	                <h3 class="card-title text-light font-weight-bold">Total Blogs </h3>

	                <button type="button" class="btn btn-secondary float-right" data-toggle="modal" @click="addBlog">Add-New <i class="nav-icon fas fa-plus ml-2"></i></button>
	              </div>
	            ...
	            <div class="modal-dialog modal-lg">
	              <div class="modal-content bg-primary">
	                <div class="modal-header modal-modify">
	                  ...
	                  <!-- form start -->
	                <form @submit.prevent="editMode ? update() : createBlog()" @keydown="form.onKeydown($event)">
	          ...
	          <div class="card-body"> 
	              <table class="table table-secondary table-bordered table-striped text-center">
	                <thead>
	                  ...
	                  <tr v-for="(result, index) in blogs" :key="result.id">
	                    <td>{{ result.id }}</td>
	                    <td>{{ result.title }}</td>
	                    <td class="description-width">{{ result.description }}</td>
	                    <td>
	                        <img :src="'/uploads/blog/'+result.image" alt="No image" style="height: 60px; width: 60px;border-radius:50%;">
	                    </td>
	                    <td>
...
<script>
    ...
    export default {
       ...
       data() {
	    ...
	    methods: {
	      	getData() {
	          	axios.get('/api/blog?page='+this.pagination.current_page)
	          	.then(response => {
	      	...
	      	searchData(){
	          	axios.get('/api/search/blog/'+this.queryField+'/'+this.query+'?page='+this.pagination.current_page)
	          	.then(response => {
		            this.blogs=response.data.data;
		            this.pagination=response.data.meta;
	          	})
	      	...
	      	createBlog() {
	          	...
	          	.then(response => {
	            	...
	            	if(this.form.successful){
	              	$('#addItem').modal('hide');
	                	toastr.success('Successfully added blog.', 'Success')
	            	}
	          	})
	      	...
	      	edit(result) {
	          	...
	          	$('#addItem').modal('show');
	          	$('#addModalLabel').text('Update Blog..');
	      	...
	      	active(id) {
	          	axios.get('/api/blog/active/'+id)

	          	.then(response => {
	            	this.getData();
	            	toastr.success('Successfully blog activated.', 'Success')
	          	})
	      	...
	      	unactive(id) {
	          	axios.get('/api/blog/unactive/'+id)

	          	.then(response => {
	              	this.getData();
	             	toastr.success('Successfully blog un-activated.', 'Success')
	          	})
      		...
      		destoryInfo() {
          		this.form.delete('/api/blog/'+this.form.id)
          		.then(response => {
            	this.getData();
            	$('#deleteModal').modal('hide');
            	if(this.form.successful){
              		toastr.success('Successfully blog deleted.', 'Success')
            	}
          	})
          	.catch(e => {
            		toastr.error('Something is wrong...!', 'Prepare to die!')
